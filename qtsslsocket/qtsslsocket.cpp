...
#define SSL_ERRORSTR() ERR_error_string(ERR_get_error(), NULL)

Q_GLOBAL_STATIC(QMutex, openssl_locks_locker)
class OpenSslLocks
{
public:
    inline OpenSslLocks()
    {
        QMutexLocker locker(openssl_locks_locker());
...
void locking_function(int mode, int lockNumber, const char * /* __file__ */, int /* __line__ */)
{
...
{
public:
    inline QtRingBuffer(int growth = 4096) : basicBlockSize(growth) {
        buffers << QByteArray();
        clear();
    }
...
QtSslSocketPrivate::QtSslSocketPrivate(QtSslSocket *qq)
    : connectionSecured(false), socket(0), shouldDeleteSocket(false),
      mode(QtSslSocket::Client), initialized(false), calledWriteToSocket(false),
...
/*!
    ...
    \e {Note: QtSslSocket requires OpenSSL. The
    ...
    conditions. The official web site is:} \l http://www.openssl.org/ \e {.
    Windows binaries are available from:} \l http://www.openssl.org/related/binaries.html \e {.}

    \e {Note: Since Qt 4.1, you can pass a QtSslSocket to QHttp by calling
...
QtSslSocket::QtSslSocket(Mode mode, QObject *parent)
    : QTcpSocket(parent), d(new QtSslSocketPrivate(this))
{
    d->mode = mode;
    d->socket = new QTcpSocket(this);
    d->shouldDeleteSocket = true;
...
/*!
    ...
    \sa pathToCACertFile(), {Guide to SSL certificates and certificate authorities}
*/
...
QString QtSslSocket::localCertificate() const
{
    ...
    // certificates.
    X509 *cert = d->ssl ? SSL_get_certificate(d->ssl) : 0;
    if (!cert)
        return "";

    char *inamestr = X509_NAME_oneline(X509_get_issuer_name(cert), 0, 0);
    QString inamelist = inamestr;
    OPENSSL_free(inamestr);

    char *snamestr = X509_NAME_oneline(X509_get_subject_name(cert), 0, 0);
    QString snamelist = snamestr;
    OPENSSL_free(snamestr);
    ...
    return QString("%1/%2/notValidBefore=%3/notValidAfter=%4/version=%5/serial=%6")
        ...
        .arg(X509_get_version(cert))
        .arg(ASN1_INTEGER_get(X509_get_serialNumber(cert)));
}
...
QString QtSslSocket::peerCertificate() const
{
    ...
    // server mode, but clients can in some cases also have
    // certificates.
    X509 *cert = d->ssl ? SSL_get_peer_certificate(d->ssl) : 0;
    if (!cert)
        return "";
...
/*!
    \reimp
*/
bool QtSslSocket::atEnd() const
{
    return QTcpSocket::atEnd() && d->readBuffer.isEmpty();
}
...
/*!
    ...
    part of the handover, but does not take ownership of the object (i.e.,
    the object is not deleted when QtSslSocket is deleted).
...
void QtSslSocket::setSocket(QTcpSocket *socket)
{
    ...
    socket->flush();
    if (d->socket && d->shouldDeleteSocket)
        delete d->socket;
    d->socket = socket;
    d->shouldDeleteSocket = false;
    d->initializeConnection(d->socket);
...
/*!
    ...
    completed. \e {It is essential that the connection is not used before the
...
void QtSslSocket::connectToHostImplementation(const QString &hostName, quint16 port, OpenMode openMode)
{
    ...
    qDebug() << "QtSslSocket::connectToHostImplementation(" << hostName
             << "," << port << "," << openMode << ")";
#endif
...
{
#ifdef QTSSLSOCKET_DEBUG
    qDebug() << "QtSslSocket::disconnectFromHostImplementation()";
#endif
...
bool QtSslSocket::initSsl()
{
    ...
    // TLSv1.
    d->ctx = SSL_CTX_new(d->mode == Server ? SSLv23_server_method() : SSLv23_client_method());
    if (!d->ctx) {
    ...
    // Initialize ciphers
    if (!d->ciph.isEmpty()) {
        if (!SSL_CTX_set_cipher_list(d->ctx, d->ciph.toLatin1().constData())) {
    ...
    if (d->mode == Server) {
        ...
        // Load certificate
        if (!SSL_CTX_use_certificate_file(d->ctx, QFile::encodeName(d->cert).constData(), SSL_FILETYPE_PEM)) {
    ...
    if (!d->cafile.isEmpty() || !d->cadir.isEmpty()) {
        ...
        if (!SSL_CTX_load_verify_locations(d->ctx,
                                           d->cafile.isEmpty() ? 0 : QFile::encodeName(d->cafile).constData(),
                                           d->cadir.isEmpty() ? 0 : QFile::encodeName(d->cadir).constData())) {
            setErrorString(tr("Failed to load the list of trusted CAs: %1").arg(SSL_ERRORSTR()));
            emit error(UnknownSocketError);
            return false;
        }
        ...
        SSL_CTX_set_verify_depth(d->ctx, 1);
#endif
    }

    // Create and initialize SSL session
    if (!(d->ssl = SSL_new(d->ctx))) {
...
void QtSslSocket::readFromSocket()
{
    ...
    if (BIO_write(d->rbio, block.data(), block.size()) != block.size()) {
    ...
    do {
        ...
        ret = SSL_read(d->ssl, buffer, sizeof(buffer));
        if (ret <= 0) {
            if (ret < 0 && SSL_get_error(d->ssl, ret) == SSL_ERROR_WANT_READ) {
                ...
                // connection.
                if (SSL_get_error(d->ssl, 0) == SSL_ERROR_ZERO_RETURN) {
#ifdef QTSSLSOCKET_DEBUG
...
void QtSslSocket::writeToSocket()
{
    ...
    if (d->connectionSecured) {
        while (!d->writeBuffer.isEmpty()) {
            int ret = SSL_write(d->ssl, d->writeBuffer.readPointer(), d->writeBuffer.nextDataBlockSize());
            if (ret <= 0) {
    ...
    int pending = BIO_pending(d->wbio);
    if (pending > 0) {
        QByteArray buffer;
        buffer.resize(pending);

        // With BIO_s_mem(), BIO_read can not fail.
        BIO_read(d->wbio, buffer.data(), buffer.size());
...
bool QtSslSocket::sslAcceptOrConnect(bool accept)
{
    ...
    int result = accept ? SSL_accept(d->ssl) : SSL_connect(d->ssl);
    if (result <= 0) {
        if (result < 0) {
    ...
    notValidBefore.setTimeSpec(Qt::UTC);
    notValidAfter.setTimeSpec(Qt::UTC);

    // X509_V_OK also if the peer had no certificate.
    int err = SSL_get_verify_result(d->ssl);
    if (err != X509_V_OK) {
